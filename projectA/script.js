...
let level=0
let obstacles = [];
let obstacleWidth = 300;
let obstacleHeight = 10;

function setup() {
  // Create canvas and initialize objects
  createCanvas(850,850);
  player = {x: width / 2, y: height / 2, speed: 5, size: 30+level*5};
  ...
  // Add monsters
  for (let i = 0; i < 7; i++) {
    monsters.push({x: random(width), y: random(height), speed: 2+level});
  }

  // Add obstacles
  for (let i = 0; i < 10; i++) {
    obstacles.push({x: random(width -50), y: random(height - 50)});
  }
}
  ...
  if (isMouseClicked) {
    ...
    for (let obstacle of obstacles) {
      ...
      obstacle.y += random(-4, 4);
        ...
        obstacle.x = constrain(obstacle.x, 20,780);
        obstacle.y = constrain(obstacle.y, 40,700);
